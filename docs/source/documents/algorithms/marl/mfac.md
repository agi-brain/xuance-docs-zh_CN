...
平均场执行器-评判器（MFAC）是一个关键的多智能体强化学习（MARL）算法，它整合了执行器-评判器框架。它引入了"平均场近似"来建模所有智能体的集体行为，而不是显式考虑每对智能体之间的交互，从而简化了大规模多智能体系统中的学习过程。

下表列出了 MFAC 算法的一些一般特性：
...
MFAC 算法通过时序差分（TD）学习更新 Q 函数，其核心思想是"使用当前估计的未来 Q 值来修正当前的 Q 值"。此时，给定经验 $e = (s, \{a^{k}\}, \{r^{j}\}, s')$，平均场 Q 函数的更新函数为：
...
MFAC 采用执行器-评判器（AC）框架。执行器-评判器方法通过执行器选择动作，通过评判器评估这些动作，并相互协作进行改进。

### 与 MFQ 的比较

就像 MFQ 一样，MFAC 也借鉴了深度强化学习中 DQN 的稳定训练技术，并采用了经验回放和目标网络的思想。然而，在策略更新方面，MFAC 使用权重为 $\theta$ 的神经网络显式建模策略。

### 更新评判器

评判器也称为价值网络，就像 MFQ 更新其 $Q$ 网络一样，MFAC 也通过以下步骤实现。

- **采样经验**：从经验回放缓冲区 $\mathcal{D}$ 中采样小批次经验 $(s, \mathbf{a}, \mathbf{r}, s', \mathbf{\bar{a}})$
- **继承平均动作**：从目标网络 $Q_{\phi^j_-}$ 中采样动作 $a^j_-$，并让目标网络继承当前的平均动作估计。

在 MFAC 中，智能体 $j$ 通过最小化损失函数进行训练：
...
执行器也称为策略网络，对于每个智能体 $j$，MFAC 提供当前策略网络 $\pi_{\theta^{j}}$ 和目标策略网络 $\pi_{{\theta^{j}}_\_}$，$\theta^j$ 和 ${\theta^{j}}_\_$ 是参数。

在状态 $s$ 下，智能体 $j$ 的动作 $a^j=\pi_{\theta^{j}}(s)$，新的平均动作 $\mathbf{\bar{a}}$ = $[\bar{a}^1, \dots, \bar{a}^N]$ 和计算公式：

$$
\bar{a}^j = \frac{1}{N^j} \sum_k a^k, a^k \sim \pi_t^k(\cdot |s,{\bar{a}^k}_\_)
$$

这里，智能体 $j$ 的 $N_j$ 个邻居来自由它们之前的平均动作 ${\bar{a}^k}_\_$ 参数化的策略 $\pi^k_t$。就像 MFQ 一样，将经验元组 $(s, \mathbf{a}, \mathbf{r}, s', \mathbf{\bar{a}})$ 存储在经验回放缓冲区 $\mathcal{D}$ 中。
...
在 XuanCe 中运行 MFAC 之前，您需要准备 conda 环境并按照
[**安装步骤**](./../../usage/installation.rst#install-xuance) 安装 ``xuance``。

### 运行内置演示

完成安装后，您可以打开 Python 控制台并使用以下命令直接运行 MFAC：
...
如果您想在 XuanCe 中未包含的自己的环境中运行 XuanCe 的 MFAC，
您需要按照
...
Agent = MFAC_Agent(config=configs, envs=envs)  # 从 XuanCe 创建 MFAC 智能体。
